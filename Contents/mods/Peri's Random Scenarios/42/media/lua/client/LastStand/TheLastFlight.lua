...
local pl = getPlayer();
		...
		if getPlayer():getHoursSurvived()<=1 then
			...
			-- Build 42 stores character stats through CharacterStat, not the
			-- removed setThirst/setHunger/setFatigue accessors.
			pl:getStats():set(CharacterStat.THIRST, 0.15); -- from 0 to 1
			pl:getStats():set(CharacterStat.HUNGER, 0.35); -- from 0 to 1
			pl:getStats():set(CharacterStat.FATIGUE, 0.35);
